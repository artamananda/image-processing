...
public class Gradient {
    public static void main(String[] args) throws IOException {
        ...
        konvolusi(mtxImg, imgRes, gradien, mtxImg.length, mtxImg[0].length);
        mtxToImg(imgRes);

    }

    public static void konvolusi(int[][] Image, int[][] ImageResult, int[] Mask,
                                 int N, int M)
//Mengkonvolusi citra Image yang berukuran N M dengan mask 3 - 3 .
    // Hasil konvolusi disimpan di dalam matriks ImageResult .
    {
        ...
        for (i = 1; i <= N - 2; i++) {
            for (j = 1; j <= M - 2; j++) {
                x[i][j] =
                        (Image[i - 1][j - 1] * Mask[0]) +
                                (Image[i - 1][j] * Mask[1]) +
                                (Image[i - 1][j + 1] * Mask[2]);
        ...
        System.out.println(Arrays.deepToString(ImageResult));
